/*
 * Draws Vex.Flow.Staves onto the canvas using the global variables
 * gl_VfStaves[] and gl_VfStaveNotes[]
 */
editor.draw = {
  // clears svg and draws all measures (whole score) again
  score: function() {
    console.log('[INFO] Redrawing score...');
    ...
    console.log("[DEBUG] " + canvasWidth);
    console.log("[DEBUG] " + canvasHeight);
    ...
    // or this:
    editor.ctx.width = editor.svgElem.clientWidth;
    editor.ctx.height = editor.svgElem.clientHeight;
    // editor.ctx.width = canvasWidth;
    // editor.ctx.height = canvasHeight;

    console.log("[DEBUG] " + editor.ctx.width);
    console.log("[DEBUG] " + editor.ctx.height);
    ...
    var numMeasures = gl_VfStaves.length;
    for(var staveIndex = 0; staveIndex < numMeasures; staveIndex++) {
      ...
      if(newLine === true || staveIndex === 0) { // TODO, fix width of newlines that contain attributes
        // gl_StaveAttributes[staveIndex].vfClef = attributes.vfClef;
        // gl_StaveAttributes[staveIndex].vfKeySpec = attributes.vfKeySpec;
        // console.log('[DEBUG] vfClef = ' + gl_StaveAttributes[staveIndex].vfClef);
        // console.log('[DEBUG] vfKeySpec = ' + gl_StaveAttributes[staveIndex].vfKeySpec);
        stave.setClef(attributes.vfClef);
        stave.setKeySignature(attributes.vfKeySpec);
        if(!newLine && attributes.vfTimeSpec) {
  ...
  measure: function(measureIndex, cursorNoteEnabled) {
    console.log('[INFO] Redrawing measure...');
    ...
    editor.ctx.openGroup("stave", "m"+measureIndex, {pointerBBox: true});
    // draw stave
    stave.draw();
    ...
    if(editor.mode === 'measure') {
      attachListenersToMeasureRect($('svg .measureRect#m'+measureIndex));

      // highlight selected measure
      if(measureIndex === selMeasureIndex) {
        $('svg .measureRect#m'+selMeasureIndex).addClass('highlightedMeasure');
      }
    }
  ...
  selectedMeasure: function(cursorNoteEnabled) {
    ...
    // console.log('[INFO] Redrawing measure ['+measureIndex+'] in note mode...');
